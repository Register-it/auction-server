...
CREATE TABLE ITEM (
  ...
  AUCTION_EXPIRATION TIMESTAMP NOT NULL,
...
CREATE TABLE BID (
  ...
  CONSTRAINT `BID_ITEM_FK` FOREIGN KEY (`ITEM_ID`) REFERENCES `ITEM` (`ID`) ON DELETE NO ACTION ON UPDATE NO ACTION
);

INSERT INTO ITEM(ID, TITLE, DESCRIPTION, AUCTION_EXPIRATION, CURRENT_PRICE, INITIAL_PRICE) VALUES
(1, 'HP Notebook', 'Un computer quasi nuovo, comprato d''occasione nel Klondyke', '2021-03-01 14:00:00', 1020.5, 1000),
(2, 'Schermo', 'Uno schermo, non c''è molto altro da dire', '2021-02-27 18:00:00', 200.0, 200.0),
(3, 'Chiavetta USB', 'Una capiente chiavetta USB, dada gift anno 2015', '2021-02-19 11:30:00', 8.75, 8.75);

INSERT INTO BID(ITEM_ID, USERNAME, AMOUNT, DATE_TIME) VALUES
